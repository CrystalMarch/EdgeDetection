...
Mat clip(Mat image, Rect tmp_rect){
    cv::Rect SrcImgROI = cv::Rect( tmp_rect.x, tmp_rect.y, tmp_rect.width, tmp_rect.height);
    cv::Mat SrcROIImg = image(SrcImgROI);
    
    cv::Size wholeSize;
    cv::Point ofs;
    SrcROIImg.locateROI(wholeSize, ofs);
    printf("ROI mat: wholeSize:(%d,%d),ofs:(%d,%d)\n",wholeSize.width,wholeSize.height,ofs.x,ofs.y);
//    SrcROIImg.adjustROI(0, -image.rows/5, -image.cols/5, 0);
//    SrcROIImg.locateROI(wholeSize,ofs);
